...
<template>
  ...
  >
    ...
    >
      ...
      <button
        v-if="isModalResize"
        @click="(modalThem = false), (isModalResize = false)"
  ...
  <div class="relative grid grid-cols-1 place-items-center">
    <div v-if="confirmModal" class="fixed top-20 z-50">
      <confirm
        ...
        @confirm="deleteAndConfirm"
      />
      <button
        @click="(confirmModal = false), (idNCC = 0)"
  ...
  <div class="flex mt-4 gap-5 relative">
    ...
    <div class="w-30 flex flex-auto justify-end gap-3">
      ...
      >
        ...
        <ul
          class="bg-white mt-1 absolute left-0 pt-2 w-full shadow-xl rounded-md pb-2 z-10"
          v-if="open"
        >
          <li
            @click="
  ...
  <div v-if="listData && isMobile === false" class="w-full mt-4 mb-5">
    <p
      v-if="result.message && result.success === false"
      class="text-red-600 bg-red-100 p-2 mb-2 rounded-xl"
    >
      {{ result.message }}
    </p>
    <p
      v-if="result.message && result.success === true"
      class="text-green-600 p-2 mb-2 rounded-md bg-green-100"
    >
      {{ result.message }}
    </p>
    <div
      ...
      :key="item.maNCC"
      class="bg-white shadow-lg rounded-lg grid grid-cols-5 gap-4 mb-3"
    >
      <!-- thông tin ncc -->
      <div class="p-3">
        <p class="bg-[#E5F2FF] text-[#007AFF] p-1 rounded-lg">
          Mã nhà cung cấp: {{ item.maNCC }}
        </p>
        <h3 class="font-bold">Tên NCC: {{ item.tenNCC }}</h3>
        <p class="text-[#7A8699]">
          Thời gian: {{ new Date(item.createAt).toLocaleDateString("vi-VN") }}
        </p>
      </div>
      <!-- Thông tin liên hệ -->
      <div class="p-3">
      ...
      <div class="p-3">
        ...
        <img
          ...
          :src="item.hinhAnh === null ? 'src/assets/NoImg.png' : item.hinhAnh"
        />
      </div>
      ...
      <div class="p-3 flex h-8/12 place-self-center gap-3 col-span-2">
        ...
        <!-- Delete -->
        <button
          @click="(confirmModal = true), (idNCCToDelete = item.maNCC)"
  ...
  <div v-else-if="listData && isMobile === true" class="w-full mt-4 mb-5">
    ...
    <div
      v-for="item in listData.data"
      :key="item.maNCC"
      class="bg-white shadow-lg rounded-lg grid grid-rows-4 gap-4 mb-3"
    >
      <!-- thông tin ncc -->
      <div class="p-2">
        <p class="bg-[#E5F2FF] text-[#007AFF] p-1 rounded-lg text-center">
          Mã nhà cung cấp: {{ item.maNCC }}
        </p>
        <h3 class="font-bold text-center">Tên NCC: {{ item.tenNCC }}</h3>
        <p class="text-[#7A8699] text-center">
          Thời gian: {{ new Date(item.createAt).toLocaleDateString("vi-VN") }}
        </p>
      </div>
      <!-- Thông tin liên hệ -->
  ...
  <div
    v-else-if="!listData && params.query !== ''"
    class="content-center w-full"
  >
    <h1 class="text-[#737373] font-semibold mt-5 text-xl">
      Không tìm thấy kết quả cho: <b class="text-black">{{ params.query }}</b>
    </h1>
  </div>
