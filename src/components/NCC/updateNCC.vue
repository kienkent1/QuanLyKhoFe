<script setup>
import {getNccById, updateNCC} from './nhaCungCap.js';
...
const nameRegex = /^[A-Za-zÀ-ỹ0-9 _-]+$/;
 ...
 const SubmitUpdateNCC = async () => {
  ...
  else if(!formData.value.DienThoai || formData.value.DienThoai.trim() === '' ){
  ...
  else if(!formData.value.Email || formData.value.Email.trim() === '' || !emailRegex.test(formData.value.Email)){
...
<div class="min-h-screen flex justify-center items-center  p-6">
      ...
      <div class="border-b flex mb-2">
        ...
        <div>
            <!-- phone -->
          <div class="mb-2">
            <label class="block mb-2 text-sm font-medium text-gray-700">Điện thoại </label>
            <input
